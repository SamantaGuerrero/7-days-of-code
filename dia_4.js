...
let mínimo = 0;
let intento = 0;
let numero = prompt("¿En qué número estoy pensando?...");
while (intento < 3){
    let numSec = numero[Math.floor(Math.random() * (máximo - mínimo + 1) + mínimo)];
    if (numSec == numero){
        alert("¡Felicidades, haz adivinado el número que tenia en mente.");
